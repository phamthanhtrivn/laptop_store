...
const EditProductModal = ({ id, fetchProductsData, isOpen, onClose }) => {
  ...
  const fetchDataProduct = async () => {
    setIsLoading(true);
    try {
      const reponse = await axios.get(backendUrl + `/api/products/get/${id}`);
      if (reponse.data.success) {
        nameRef.current.value = reponse.data.product.name;
        priceRef.current.value = reponse.data.product.price;
        descRef.current.value = reponse.data.product.description;
        stockRef.current.value = reponse.data.product.stock;
        cpuRef.current.value = reponse.data.product.specs.cpu;
        ramRef.current.value = reponse.data.product.specs.ram;
        storageRef.current.value = reponse.data.product.specs.storage;
        gpuRef.current.value = reponse.data.product.specs.gpu;
        screenRef.current.value = reponse.data.product.specs.screen;
        setSelectedBrand(reponse.data.product.brand);
        setSelectedCate(reponse.data.product.category);
        setImage1(reponse.data.product.images[0]);
        setImage2(reponse.data.product.images[1]);
        setImage3(reponse.data.product.images[2]);
        setImage4(reponse.data.product.images[3]);
      }
    } catch (error) {
